...
> **Vendored fork (bevy-boids).** Based on upstream 0.14.0. The
> per-frame `follow_ground` mesh raycast is removed; the host terrain system
> sets `RtsCamera::target_focus.translation.y` analytically before
> `RtsCameraSystemSet`. `Ground` now only marks drag-pan grab anchors.
...
- Focus height is host-supplied (this fork has no per-frame ground raycast)
- Smoothed movement
...
Optionally add `Ground` to your ground/terrain entities if you use
`RtsCameraControls::lock_on_drag` (it marks the meshes a drag can grab):
