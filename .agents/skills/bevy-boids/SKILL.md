...
| Module | Contents |
| --- | --- |
| `main.rs` | App assembly, all schedules, `setup` (shared handles, gizmo depth-bias, and the bare **app camera**), the camera rig fns (`reset_app_camera_rts` / `configure_app_camera_plain` / `apply_camera_dressing` / `strip_camera_dressing`), and the unconditional `TerrainMesh` + `ErosionDemoPlugin` wiring so the normal world can reload from any launch mode. **The app camera (`AppCamera` marker) is spawned ONCE and never despawns or replaced**: bevy_egui attaches its primary context (every menu) to the first camera and never attaches another for the app's lifetime, and the context must ride a camera for egui's render extraction — so world switches RECONFIGURE this one entity (fresh RTS rig, or the billboard scene's plain pose) instead of respawning it. Moving the context between cameras churns egui's internal state (phantom clicks, an emath time-monotonicity panic by the fifth load) — don't.
...
| `scene.rs` | The world assembler + standalone test scenes (`--scene <name>`, `ScenePlugin`). Exactly one "world" is loaded at a time — the normal RTS sandbox or one test scene — tracked by `ActiveScene(Option<TestScene>)` (seeded from `--scene`; `in_scene(variant)` is its run-condition factory, `normal_world()` the None-side twin). Every load, startup and runtime alike, goes through `assemble_world` (invoked by `load_world`, an EXCLUSIVE system — atomic, nothing concurrent; WARNING: exclusivity does NOT flush other systems' pending command buffers (the executor only flushes at sync points, so a pending `swap_boid_lod` buffer outliving the teardown panicked "Entity despawned" on F4 loads); the guards are main.rs's Playing Update tuple `.before(load_world)` (edges from deferred systems insert auto sync points) plus billboard conversions AND `Selected`'s on_remove hook checking entity validity at buffer-application time (the hook fires mid-despawn during `teardown_world` itself — its queued indicator cleanup applies after `load_world` returns, so ordering alone cannot fix that one): `teardown_world` despawns all world-owned entities (boids/formations/obstacles/grounds/crowd props via one `Or<With<..>>` filter — never the camera; see the `main.rs` row for why) and resets `Player`/`BoidIds`/`CameraMode`, then the target assembles — the sandbox (grid, obstacle scatter, RTS rig + launch dressing, erosion ground + water or the `--flat` plane) or the scene (props + its ground + a fresh undressed rig via `reset_app_camera_rts`/`configure_app_camera_plain`; `DebugConfig.impostor_lod` stands the billboard auto-swap down in scenes). `LoadWorld(Option<TestScene>)` is the UI request resource (F4 menu / main-menu picker / Start), consumed by `load_world` (Update + the Startup copy main.rs chains after `setup`). `wants_flat_ground()` picks flat-plane vs crowd hills. Movement scenes (`arrival`, `perpendicular`, `head-on`, `clash`, `shove`, `melee`, `formation-cross`, `formation-braid` (the head-on jam stress test), `formation-clash` — growing PBD-collision complexity; plus `formation-parade`: one formation of `SceneUnits.parade_units` (F4 slider, default 20) cycling every `FormationKind` around a triangular course — per leg `Reform` into the next kind → `Move` onto the corner → `Rotate` toward the next one, the queue built by `parade_orders` from `FormationKind::ALL` so new kinds extend the parade with no scene-code change) share helpers: `spawn_scene_boid` (ids pick faction-matching colours: `id = faction + 3·seq`; the counter resets per load, so reloads are bit-identical), `spawn_marching_block` (Grid block + `Move` order, member placement shared via `spawn_block_members`; block size = `SceneUnits.per_formation` — near-square, default 12 = the historical 4×3, set from the F4 slider — and the formation scenes scale their stage by `stage_scale` = √(count/12) so big blocks don't spawn pre-interpenetrated), `scene_camera` (fresh RTS rig on the app camera, pinned via `SceneCameraPose`, consumed and dropped by `pin_scene_camera`; `height_m` converts to zoom units where 1.0 = 2 m and 0.0 = 30 km). The `billboard` scene: a mesh capsule (screen LEFT) beside its billboard twin (screen RIGHT; `billboard_scene_convert` guards on `Without<Billboard>` so reloads reconvert). The `crowd` scene: viewed through the game RTS camera; its armies are a CrowdPlugin Update system gated on `in_scene(Crowd)` + shader-module load, presence-guarded so world reloads respawn them; its ground is `crowd::spawn_crowd_ground`. The module docs list the steps to add a scene (variant → spawn fn → `assemble_scene` match arm). Camera persistence is test-pinned (`world_switches_keep_one_persistent_camera_for_egui`). Visual captures: `--bench --scene=<name> --zoom ~0.998 --cam-angle 25 --shot <path>` (the shot fires at `--secs` − 5 s, so pass `--secs 12` to catch t≈7 s) |
| `ui/scenes.rs` | `ScenesUiPlugin`: the scene picker — one egui window, opened by F4 while playing (`ToggleScenes` action) or the main menu's "Scenes" button. Lists every `TestScene` (current one marked) + "Reset current scene"; picks insert `scene::LoadWorld`, and from the main menu also enter `Playing`. A units-per-formation slider (1..=10k, default 12) and a parade-units slider (default 20) edit `scene::SceneUnits` — the marching-block size the formation scenes load with, and the formation-parade scene's single formation. After any click it surrenders egui keyboard focus — a clicked egui button keeps focus otherwise, and while egui wants the keyboard the F1-F4 toggles stand down. Esc closes it before pausing (see `toggle_pause`) |
| `ui/help.rs` | `HelpUiPlugin`: the Help window — a gesture-oriented controls reference (drags vs clicks, drag modifiers like additive-select and width-fitting frontages, the F1/F3/F4 panels; camera keys are static text since the RTS rig isn't rebindable). Opened by Help buttons on the main and pause menus (`HelpOpen` resource, same pattern as `OptionsOpen`); Esc closes it via `toggle_pause`. Rebindable rows render the *current* bindings via `input::current_binding` (the pub lookup shared with the Key Bindings window), so remaps keep the text accurate |
...
| `boid.rs` | `Boid { id }` (stable spawn-order identity from `BoidIds`; `variation_for(id)` deterministically picks the visual variation — soldiers never "switch places" across LOD swaps or future streaming), the `BoidVariations` catalog (`FromWorld`; three tinted-capsule placeholders prove the machinery; the baker and spawner share this ONE list), `BoidBundle::with_id` (includes `Faction` + `Body` for the PBD solver), `bob` (STATELESS cosmetic: three fixed cadences — idle/walk/run — blended by speed, each a pure `sin` of global time so a speed change morphs weights and can never jump the phase; the per-boid de-sync is hashed from `Boid::id`, no per-boid component — the accumulated-phase form this replaced read as acceleration-driven chatter), `BoidTuning` |
...
| `kinematics.rs` | `Velocity { v, a, target_v, slope, slope_col }` (slope = cached uphill gradient, resampled per metre column; collision response is positional in `pbd.rs`, so there is no push-velocity field), tuning consts + `KinematicsTuning`, `arrival_plan` (arrival-damped, misalignment-slowed preferred velocity — the anti-orbit fix), slope-aware `move_step` (downhill gravity, uphill thrust loss, downhill cap relaxation; sheds cap excess with bounded decel, never snaps), `NNTree`/`TrackedByTree`, `HardCollision` obstacle marker |
| `target.rs` | `Target { pos, dir, speed_scale, soft_arrival_m }` (scale multiplies the tuning velocity cap — Walk/Run; soft_arrival fades steering authority near the target — formation slots set it, see `FormationTuning::slot_soft_radius_m`), `follow_target` desired-velocity steering (brakes toward the preferred velocity; accel clamp scaled by `kinematics::arrival_authority`) |
| `pbd.rs` | Position-based collision response (Weiss et al. MiG 2017, arXiv:1802.02673 — design record in `docs/pbd-anticipation.md`): `pbd_contact` (Update, `.after(move_step).before(ground_boids)`) projects boids out of overlap instead of adding forces — frictional contact with inverse-mass weighting (`Body { radius_m, mass_kg }` — heavy units get shoved less), Macklin-style positional friction (cone-clamped, no velocity reads), long-range time-to-collision anticipation for friendly pairs (`time_to_collision` quadratic; the §4.5 sidestep sheds the braking component so units slide past instead of halting), and infinite-mass obstacle-cuboid seating. Velocity picks corrections up as `Δv = Δx/Δt` at write-back. Clash mode: `Faction` (different ids hostile, symmetric) keeps contact but drops anticipation — bodies slam and grind; `Charging` drops anticipation against everyone. Solver: kd-tree candidate gather once per step (`k_nearest` + margin; default 12 candidates × 1 Jacobi pass — hand-tuned, wider coverage beats re-relaxation), Jacobi iterations over a reused flat SoA scratch (`PbdScratch`, double-buffered positions, `ComputeTaskPool::scope` chunk-parallel) — 0.8 ms at 10k boids in release (budget test `pbd_contact_scales_to_10k_boids`). Pure fns unit-tested; integration tests pin non-penetration, clash-hold, shove-by-mass, and the three-faction melee bound |
| `formations.rs` | `Formation`, `FormationKind` (Line/Column/Grid/Wedge/Ring, catalogued in `FormationKind::ALL` for iterating code — keep the array in sync when adding a variant), `FormationSlot`, relationship components, `FormationOrder` queue, `SlotsStale`/`FormationGoal` message components, the chained executor pipeline (`transition_formation_orders`, `plan_formation_goals`, `dispatch_formation_goals`), Morton-order slot assignment, LOD, `FormationTuning`, most tests |
...
| `ui/mod.rs` | `UiPlugin`: egui (`bevy_egui`), `GameState { MainMenu, Playing, Paused }`, main/pause menus (main menu's Start requests the normal world back via `scene::LoadWorld` when a scene is loaded), Options menu (`OptionsSettings` persisted via bevy-settings, `apply_options` pushes to live values), terrain-tuning panel on F3 (world-gen sliders + view modes, visibility = `TerrainPanelOpen` resource), pause plumbing (`Time<Virtual>` + camera kill-switch). `toggle_pause` is the universal Esc cascade: it closes the topmost window *visible in the current state* (playing: radial → Scenes → F1 debug → F3 terrain; menus: Key Bindings → Options → Help → Scenes) before pausing/resuming — stale flags from another state don't swallow the press; the main menu itself is never "closed" |
| `ui/debug.rs` | `DebugUiPlugin`: F1 debug panel — sliders over the `*Tuning` resources, gizmo/LOD/sky toggles, FPS; `DebugConfig` consumed by gizmo systems. The Camera section holds the freecam toggle + fly-speed slider (see `freecam.rs`) and the RTS pan-speed slider |
| `freecam.rs` | `CameraMode { Rts, Free }` resource (toggled from the F1 debug panel's Camera section) + `Freecam`: free-fly camera. `apply_camera_mode` (gated `resource_changed`) swaps by component presence — freecam removes `RtsCamera`/`RtsCameraControls` (standing the crate's systems down) parked on `Freecam::saved` for restoration; `freecam_move` is WASD/QE/RMB-drag/wheel input, inert without a `Freecam` camera; `Freecam::speed_mps` (the base fly speed) is public — the wheel and the F1 fly-speed slider (logarithmic, bounded by `FREECAM_MIN/MAX_SPEED_MPS`) both retune it |
| `terrain/` | THE terrain: the `bevy_erosion_filter` terrain demo recreated on the CPU over a flat-substrate 1 km² square (`demo.rs`, no LOD): a 3-octave gain-0.1 fBm base + the crate's `cpu::erosion_filter` carve gullies and the ridge map, the demo's albedo cascade (cliff/dirt/snow/sand/grass/trees/drainage) rides vertex colors on the 256×256 `TerrainMesh` grid, a translucent `WaterPlane` rides `water_level`, and the demo's 22 m world stretches uniformly by `WORLD_SCALE` (1000/22) so slopes and gully sizes match the reference at game scale. `DemoTerrain` (F3 panel: filter sliders, terrain scales, four view modes) marks dirty; `rebuild_terrain` re-evaluates all 66k vertices (~160 ms — deliberately on slider RELEASE, drags coalesce) and rebuilds the closure-backed `HeightField` from the same evaluation, so grounding (`grounding.rs`, `GroundY`), camera focus/clearance (`camera.rs`), obstacle seating and the cursor raycast agree with the render by construction. The erosion ground and water are spawned by the world assembler (`spawn_erosion_ground` + `demo::spawn_water`, both `&mut World` callables); `rebuild_terrain` is gated on `scene::normal_world` — scenes install their own height fields. Barebones pieces in `mod.rs` (`grid_mesh`, `ObstacleBundle`); `rebuild_mesh`'s buffers-then-attributes shape is a borrow-checker requirement. The streamed-tile LOD system is deleted (`tiles.rs`/`noise.rs`/`render.rs`, see the a84a4f4 reset) |
