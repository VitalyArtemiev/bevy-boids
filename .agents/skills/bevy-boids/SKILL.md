...
| `terrain/` | THE terrain, currently barebones after the LOD reset: one flat 1 km² square (`TERRAIN_EXTENT_M`, a 256×256-quad grid mesh in the `TerrainMesh` resource, spawned by `spawn_ground`), a closure-backed `HeightField` (flat by default) feeding boid grounding (`grounding.rs`, `GroundY`), camera focus-follow + terrain clearance (`camera.rs`, `CameraClearance`), obstacle seating (`ObstacleBundle`, `project_obstacles_onto_field`) and the player's cursor raycast. The streamed-tile LOD system, the noise generator and the displaced-mesh render path were removed wholesale (`tiles.rs`/`noise.rs`/`render.rs` deleted); next milestone is the `bevy_erosion_filter` CPU example recreated on this square without LOD |
