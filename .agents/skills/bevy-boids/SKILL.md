...
| `terrain/` | THE terrain: `HeightField` resource (authoritative height fn, closure-backed, returns `TerrainSample` with height/slope/ridge map, carries the mountain-scale relief for unit-height color gates), heights in `noise.rs` from three analytic layers: a **tectonic Worley layer** (plate cells ~km; plate space runs through a seeded curl warp so boundaries meander instead of following straight bisectors; plate DRIFT comes from a smooth circulation field — coherent margins produce long ranges/rifts, per-plate random drifts degenerate to speckles; per-boundary strength averages the two plates' hashed activities; colliding boundaries raise relief toward mountains, splitting ones carve damped rift basins, shear stays quiet; relief AND feature scale are per-point functions of it — keep the closed-form gradients in sync with `tectonic_gradients_match_finite_differences`), a **multi-octave base landform** at world wavelength `base_wavelength_m` (gain 0.45 — the demo's gain-0.1 fBm is effectively single-octave and turned scaled mountains into uniform high-frequency peaks) whose world-metre displacement is 0.35× local relief, plus an orogeny-gated **ridged macro layer** (1.8 km massif crests), all under the runevision Advanced Terrain Erosion Filter via the `bevy_erosion_filter` cpu port (the filter keeps the reference-demo unit-height input semantics; do NOT feed it big amplitudes — its `onset` gate is tuned for gentle unit slopes, and seed offsets MUST stay within ±16 p-units — the crate's IQ hash degenerates to a constant gradient at large coordinates, which once produced uniform terrain with orthogonal ridges), streamed texture-backed tiles in LOD rings (UDLOD-style; `tiles.rs` bakes + `render.rs` draws, 1 m cells near camera to 65 km at continental distance; rings always centre on the RTS focus — centring on the camera body thrashes the stamps because the RTS camera slides ~11 km in XZ per zoom gesture — while `LodMode` gates the finest rendered level by camera distance, `FinestAtFocus` being the F3 debug mode). All tiles share ONE mesh — a flat 33×33 grid in texel coordinates, deliberately NO skirt ring (rim stitching + the coarse fringe's overlap already seal level seams; hanging walls showed up as coincident z-fighting polygons at ring boundaries) — and every per-tile fact lives in the shared `TileAtlas`: three `Texture2DArray`s (final heights R32Float with level drop + the rim MORPH BAND baked in (rim tiles blend onto the parent surface across the outer 8 quads via a 9×9 parent lattice — the LOD boundary is a continuous ramp; edge-only stitching left the parent's boundary strip free to tower over the fine ring's valleys and occlude them), 3×3-blurred slopes RG32F, per-vertex tints RGBA8), 2048 layers each, one layer per live-or-waiting-or-cached tile handed out by `TileAtlas::alloc` and returned on cache eviction/field rebuilds; when a wave would overclaim, the streaming system first evicts cached renders (`TileRenderCache::evict_oldest`) — only live tiles beyond the bound assert. `TerrainMaterial = ExtendedMaterial<StandardMaterial, TerrainExtension>` places the tile uniform at binding 200 and the four array textures at 201-204 (indices must match `assets/shaders/terrain_common.wgsl` exactly); the layer index rides `shading.w`. The extension overrides `vertex_shader()`/`prepass_vertex_shader()` with `terrain.wgsl`/`terrain_prepass.wgsl` (both import `terrain_common.wgsl` by namespace), while the fragment stays StandardMaterial's stock lighting. The vertex shader displaces and cuts the clipmap hole by clamping interior verts onto the hole rectangle (degenerating exactly the quads the CPU mesh used to drop from the index buffer). **No time-animated LOD fades** — geometry is static per bake (a time-morphed whole-tile surface visibly undulated on every restamp and blurred tiles waiting to retire; pinned by `hole_cutters_never_expose_their_hole`): spawn order is finest-first so a hole-cutter's fine coverage is always already live, and retirement waits for coverage (`renders_at`), so neither side of a transition opens a void. Tiles spawn with `NoFrustumCulling` (the shared mesh is displaced in-shader, so a CPU AABB would cull with the wrong box). **Load-bearing gotcha**: `terrain_common.wgsl` is imported by namespace, and naga-oil resolves namespace imports only against *loaded* shader assets — `TerrainRenderPlugin` loads it and parks the handle in the `TerrainCommonShader` resource; remove that and the terrain silently never draws (no error — the pipeline retries forever). Streaming never opens holes: stale tiles render until their replacement spawns in the same frame (atomic swap), and gate-dropped tiles wait until a current tile renders over them. A tile bake costs ~2 ms release (~4 ms dev) of field sampling serially, so `stream_terrain_tiles` bakes `StreamBudget`-capped (2 ms) waves of tiles in parallel via `std::thread::scope` (serial fallback on wasm); a bake write patches its atlas layer in the array `Image`s, and the asset change re-uploads through `COPY_DST` batched once per frame per array (a render-world per-layer `write_texture` would shave the startup storm if it ever matters). Retired renders (material + atlas layer) park in the `TileRenderCache` LRU (256 tiles) for zero-cost re-spawn. Also boid grounding (`grounding.rs`, `GroundY`), camera focus-follow + terrain clearance (`camera.rs`, `CameraClearance`), obstacles |
