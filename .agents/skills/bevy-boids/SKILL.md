...
- Bench: `cargo run -- --bench` (plays 30 s with no main menu, camera
  ...
  `--flat` (the same bare ground scene without the crowd armies — for
  render-path benches), `--force-meshes`/`--force-billboards` (hold every
  ...
  live), `--billboard-lab` (implies `--flat`, suppresses the RTS camera
  and grid boids: one forced-billboard boid beside its mesh twin up close
  under the same sun — pair with the sun flags and `--shot` to compare
  the render paths). Render-path A/B recipe:
