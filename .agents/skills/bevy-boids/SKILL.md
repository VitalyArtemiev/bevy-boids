...
- Bench: `cargo run -- --bench` (plays 30 s with no main menu, camera
  ...
  `--shot` to compare the render paths; see `scene.rs` to add more). Render-path A/B recipe:
